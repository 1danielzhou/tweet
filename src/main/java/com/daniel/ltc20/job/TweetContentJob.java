...
import com.daniel.ltc20.utils.TelegramApiUtil;
...
public class TweetContentJob {
    ...
    @Autowired
    private TelegramApiUtil telegramApiUtil;
    ...
    public void getTweetUrl() {
        ...
        if (ObjectUtil.isNotEmpty(tweetSearchKeyword)) {
            ...
            telegramApiUtil.sendMessage(StrUtil.format("关键词{}的Urls在{}小时内未更新，下面尝试进行更新操作,{}", tweetSearchKeyword.getKeyword(), INTERVAL, tweetSearchKeyword));
            ...
            if (CollUtil.isEmpty(tweetUrls)) {
                ...
                telegramApiUtil.sendMessage(StrUtil.format("获取Url失败或者获取为空，将lastUrlUpdateTime还原回：{},以便再次获取", tweetSearchKeyword.getLastUrlUpdateTime()));
                ...
                telegramApiUtil.sendMessage(StrUtil.format("一共获取到{}个链接，将其插入数据库中", tweetUrls.size()));
            ...
            telegramApiUtil.sendMessage(StrUtil.format("没有查询到在过去{}小时未更新的关键词", INTERVAL));
        }
    }
    ...
    public void searchStoreYesterdayTweet() {
        ...
        if (ObjectUtil.isNotEmpty(tweetSearchKeyword)) {
            ...
            telegramApiUtil.sendMessage(StrUtil.format("关键词{}的Urls在24小时内未收集最新的Tweet，下面尝试进行收集,{}", tweetSearchKeyword.getKeyword(), tweetSearchKeyword));
            ...
            if (!result) {
                ...
                telegramApiUtil.sendMessage(StrUtil.format("关键词{}收集昨天的Tweet失败，将last_collect_data_time还原回：{},以便再次获取", tweetSearchKeyword.getKeyword(), tweetSearchKeyword.getLastCollectDataTime()));
                ...
                telegramApiUtil.sendMessage(StrUtil.format("收集昨天新发的tweet结束，更新结束的时间"));
            }
        } else {
            log.info("没有查询到在过去{}小时未更新前一天数据的关键词", 24);
            telegramApiUtil.sendMessage(StrUtil.format("没有查询到在过去{}小时未更新前一天数据的关键词", 24));
        }
    }
    ...
    public void refreshHistoricalTweet() {
        ...
        if (ObjectUtil.isNotEmpty(tweetSearchKeyword)) {
            ...
            telegramApiUtil.sendMessage(StrUtil.format("关键词{}的还未更新过去7天的历史数据，下面尝试进行收集,{}", tweetSearchKeyword.getKeyword(), tweetSearchKeyword));
            ...
            if (!result) {
                ...
                telegramApiUtil.sendMessage(StrUtil.format("关键词{}更新过去7天的Tweet失败，将last_refresh_historical_data_time还原回：{},以便再次获取", tweetSearchKeyword.getKeyword(), tweetSearchKeyword.getLastRefreshHistoricalDataTime()));
                ...
                log.info("关键词{}过去7天的Tweet更新成功", tweetSearchKeyword.getKeyword());
                telegramApiUtil.sendMessage(StrUtil.format("关键词{}过去7天的Tweet更新成功", tweetSearchKeyword.getKeyword()));
            }
        } else {
            log.info("没有查询到在过去{}小时未更新过去7天历史数据的关键词", 24);
            telegramApiUtil.sendMessage(StrUtil.format("没有查询到在过去{}小时未更新过去7天历史数据的关键词", 24));
        }
    }
    ...
    public void postProcessTweet() {
        ...
        if (ObjectUtil.isNotEmpty(tweetSearchKeyword)) {
            ...
            telegramApiUtil.sendMessage(StrUtil.format("查询到一个还没有进行后处理的keyword:{}，下面进行后处理,{}", tweetSearchKeyword.getKeyword(), tweetSearchKeyword));
            ...
            try {
                ...
                telegramApiUtil.sendMessage(StrUtil.format("keyword:{}的post process 结束，", tweetSearchKeyword.getKeyword()));
            } catch (Exception e) {
            ...
            telegramApiUtil.sendMessage("没有查询到需要post process的关键词");
        }
    }
}
