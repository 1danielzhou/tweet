...
public class TweetController {
    ...
    public String init(String searchKey) {
        if (!tweetSearchKeywordService.setInitStatus(searchKey)) {
            log.info("设置标志位失败，{}正在进行初始化操作，或者初始化操作已经结束", searchKey);
            return StrUtil.format("{}正在进行初始化操作，或者初始化操作已经结束", searchKey);
        }
        ...
        tweetSearchKeywordService.updateInfo(searchKey, size);
